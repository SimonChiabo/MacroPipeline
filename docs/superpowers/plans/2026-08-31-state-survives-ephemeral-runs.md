...
**COMPLETADO el 2026-08-31.** CI verde run `33403182196` sobre el HEAD exacto `6855eb3`; 283 tests, Codecov 90.97%.
...
- [x] **Step 3:** Tras el rename, volver a correr `_init_db()` y `_migrate_db()` sobre el fichero bajado — corrieron sobre el vacío, no sobre este. Son idempotentes a propósito.

- [x] **Step 4:** `./.venv/Scripts/python.exe -m pytest tests/unit/test_state_sync.py -q`
...
- [x] **Step 3: Verificación por mutación**
...
- [x] **Step 4:** `./.venv/Scripts/python.exe -m pytest tests/unit/test_state.py -q`
...
- [x] **Step 3:** `./.venv/Scripts/python.exe -m pytest tests/ -q` y `./.venv/Scripts/python.exe -m mypy src`
...
- [x] **Step 1: Partir la fila de R2 en la tabla de ADR-009**
...
- [x] **Step 2:** ADR-002: su implicación de resiliencia ahora depende de que el estado se sincronice. ADR-007: la reproducibilidad también.

- [x] **Step 3: Recorrer la tabla de ADR-009 fila por fila contra el código.**
...
- [x] **Step 1:** `.env.example` y `README.md`: R2 pasa a sostener el estado, no solo los snapshots.
- [x] **Step 2:** CI verde sobre el HEAD exacto — el run, no "CI configurado". Anotar número de run, conteo de tests y cobertura.
- [x] **Step 3:** Actualizar `macropipeline-pending-work.md`: cerrar el punto 11, y anotar aparte las dos cosas que quedan abiertas a propósito — la alerta sobre `in_progress` viejo y los secrets de R2 en el futuro workflow programado.
