...
class ValidationEngine:
    """
    Motor que aplica las reglas declarativas (YAML) a los esquemas validados
    de Pydantic.
    ...
    def validate_weekly_close(self, data: WeeklyCloseData) -> bool:
        ...
        if not (sp500_min <= data.sp500_weekly_return <= sp500_max):
            ...
            raise ValidationError(
                f"Retorno del SP500 {data.sp500_weekly_return} fuera del "
                "rango permitido."
            )
        ...
        if not (nasdaq_min <= data.nasdaq_weekly_return <= nasdaq_max):
            ...
            raise ValidationError(
                f"Retorno del NASDAQ {data.nasdaq_weekly_return} fuera del "
                "rango permitido."
            )
    ...
    def validate_macro_snapshot(
        ...
        checks = [
            (
                "IPC interanual",
                data.cpi_yoy,
                rules.get("cpi_yoy_min", -1.0),
                rules.get("cpi_yoy_max", 1.0),
            ),
            (
                "Tasa de desempleo",
                data.unemployment_rate,
                rules.get("unrate_min", 0.0),
                rules.get("unrate_max", 100.0),
            ),
            (
                "Rendimiento del Treasury 10Y",
                data.treasury_10y,
                rules.get("dgs10_min", 0.0),
                rules.get("dgs10_max", 100.0),
            ),
        ]
        for label, value, minimum, maximum in checks:
            if not (minimum <= value <= maximum):
                logger.error(
                    "validation_failed",
                    reason="macro_value_out_of_bounds",
                    indicator=label,
                    value=value,
                )
                raise ValidationError(
    ...
    def validate_macro_release(self, data: MacroReleaseData) -> bool:
        ...
        if data.indicator_name.upper() == "GDP":
            ...
            if abs(data.actual) > gdp_max_abs:
                logger.error(
                    "validation_failed", reason="gdp_growth_anomaly", value=data.actual
                )
                raise ValidationError(
                    f"Cambio en GDP del {data.actual} parece poco realista o erróneo."
                )
        ...
        if data.indicator_name.upper() == "UNRATE":
            ...
            if not (unrate_min <= data.actual <= unrate_max):
                logger.error(
                    "validation_failed", reason="unrate_anomaly", value=data.actual
                )
                raise ValidationError(
                    f"Tasa de desempleo {data.actual} fuera de rangos racionales."
                )

        logger.info(
            "macro_release_validated",
