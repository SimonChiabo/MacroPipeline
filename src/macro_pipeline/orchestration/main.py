...
class MacroOrchestrator:
    ...
    def _fetch_weekly_close(self) -> WeeklyCloseData:
        ...
        # Utilizar Alpha Vantage con SPY y QQQ como proxy debido a restricciones 403 en FMP
        try:
            ...
            try:
                ...
                logger.warning("av_failed_using_mock_data", error=str(av_error))
                # Fallback final a Mock Data para poder probar todo el pipeline de publicación e IA
                dates = pd.date_range(end=pd.Timestamp.today(), periods=10, freq="B")
                sp500_df = pd.DataFrame({"date": dates, "close": [5100 + i*10 for i in range(10)]})
                nasdaq_df = pd.DataFrame({"date": dates, "close": [16000 + i*30 for i in range(10)]})
    ...
    def run_weekly_close(self):
        ...
        span_context = self.tracer.start_as_current_span("weekly_close_pipeline") if self.tracer else nullcontext()
        ...
        with span_context as span:
            try:
                ...
                # Si estamos en un span válido, documentar el error en Grafana
...
if __name__ == "__main__":
    ...
    # Cargar variables de entorno desde .env local, sobrescribiendo el caché de la terminal
    load_dotenv(override=True)
    
    # Configuración básica de la consola para stdlib (requerido por LoggerFactory)
