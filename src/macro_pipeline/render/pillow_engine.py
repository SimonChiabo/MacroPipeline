...
from macro_pipeline.validators.schemas import EarningsData, MacroReleaseData
...
class PillowEngine:
    ...
    def render_earnings_calendar(self, data: EarningsData) -> bytes:
        ...
        for report in data.reports[:10]:  # Limitar a 10 para que quepa verticalmente
            draw.text((60, y_offset), report.symbol, font=font_body, fill=self.text_color)
            
            eps_actual = f"${report.eps_actual:.2f}"
            eps_est = f"${report.eps_estimated:.2f}" if report.eps_estimated else "N/A"
            
            # Color basado en superación de expectativas (Verde si supera/iguala, Rojo si falla)
            if report.eps_estimated:
                actual_color = (74, 222, 128) if report.eps_actual >= report.eps_estimated else (248, 113, 113)
            else:
        ...
        if len(data.reports) > 10:
            draw.text((60, y_offset), f"... y {len(data.reports) - 10} mas", font=font_body, fill=self.text_color)
