...
def test_send_approval_request_with_image(mock_post, tg_env):
    ...
    with patch.dict(os.environ, tg_env):
        bot = TelegramBot()
        msg_id = bot.send_approval_request("Draft con imagen", image_bytes=b"fake_image_bytes")
...
def test_wait_for_approval_success(mock_post, mock_get, tg_env):
    ...
    # Deberían haber 2 posts de mantenimiento (answerCallbackQuery y editMessageReplyMarkup)
